...
for train_index, test_index in skf.split(np.stack(df.X.values), df.risk.values):
    ...
    for iteration in np.append(train.rep_rank.min() - 1, train.rep_rank.sort_values().values):
        ...
        if len(X_train_selected) - 2 >= (len(X_train_selected) + len(certain)) * 0.2:
            print(
                f'{len(X_train_selected)} selected and {len(certain)} so selecting {int((len(X_train_selected) + len(certain)) * 0.2)} for validation from selected.')
            X_train_selected, X_valid, y_train_selected, y_valid = train_test_split(X_train_selected, y_train_selected,
                                                                                    test_size=int((
                                                                                                              len(X_train_selected) + len(
                                                                                                          certain)) * 0.2),
                                                                                    random_state=kseed,
            ...
            print(
                f'{len(X_train_selected)} selected and {len(certain)} so selecting {len(X_train_selected) - remaining_size} for validation from selected.')
            X_valid, y_valid = X_train_selected[:(len(X_train_selected) - remaining_size)], y_train_selected[:(
                        len(X_train_selected) - remaining_size)]
            X_train_selected, y_train_selected = X_train_selected[
                                                 (len(X_train_selected) - remaining_size):], y_train_selected[(
                                                                                                                          len(X_train_selected) - remaining_size):]
...
test_scores.to_pickle('results_uncertainty_criterion_ablation.pkl')
